...
- **A_LinearStructures.a_Arrays store data continuously in memory**, so **even if we have the exact available space in memory, if it's not aligned, it won't be able to allocate.**
- **A_LinearStructures.a_Arrays only accept objects of the same type** in memory.
- **A_LinearStructures.a_Arrays have **read time O(1) and write time O(n)** in most cases, as they must move all elements to write to a place where something already exists.**
- **A_LinearStructures.a_Arrays have a fixed size**, so they can have empty elements or insufficient space.
...
## When to Use A_LinearStructures.a_Arrays

**Use A_LinearStructures.a_Arrays when:**
...
**Avoid A_LinearStructures.a_Arrays when:**
